...
@authentication_classes([])
@permission_classes([AllowAny])
def delete_cookie_and_redirect(request):
    match request.method:
        case "GET":
            response: Response = redirect(settings.REDIRECT_URL)
            response.delete_cookie(
                "devclad-refresh", domain=settings.SESSION_COOKIE_DOMAIN, path="/"
            )
            response.delete_cookie(
                "devclad-token", domain=settings.SESSION_COOKIE_DOMAIN
            )
        case _:
            return Response({"error": "Invalid method"}, status=405)
    return response
...
def get_initial_data(request):
    match request.method:
        case "GET":
            try:
                # return http only cookie as json
                # read cookie via get request from a different domain

                refresh_token: str = request.COOKIES["devclad-refresh"]
                access_token = request.COOKIES.get("devclad-token")
                return Response({"refresh": refresh_token, "token": access_token})
            except KeyError:
                return Response({"error": "No cookie found"}, status=404)
        case _:
            return Response({"error": "Invalid method"}, status=405)
