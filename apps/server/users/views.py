...
def set_cookie_and_redirect(request):
    match request.method:
        case "GET":
            ...
            response.set_cookie("devclad-refresh", request.GET["refresh"])
            response.set_cookie("devclad-token", request.GET["token"])
        case _:
            return Response({"error": "Invalid method"}, status=405)
    return response
...
def delete_cookie_and_redirect(request):
    match request.method:
        case "GET":
            response: Response = redirect("http://localhost:5173")
            response.delete_cookie("devclad-refresh")
            response.delete_cookie("devclad-token")
        case _:
            return Response({"error": "Invalid method"}, status=405)
    return response
